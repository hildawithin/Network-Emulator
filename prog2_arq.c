...
struct pkt buffera;
int altbit = 0;
float timerinc = 1000.0;

int char_checksum(char payload[20])
{
    int sum = 0;
    // if (sizeof(payload)/sizeof(payload[0])==0)
    // {
    //     return 0;
    // }
    for (int i = 0; i < 20; i++)
...
{
  //(void)message;
  // check if there is outstanding unacked msg
  if (altbit == 1)
  {
      printf("Ignore message due to outstanding unacked message.\n");
      return 0;
  }
...
{
  //(void)packet;
  // check checksum and send to layer 5 curacknuma == packet.acknum && curseqnuma == packet.seqnum-1 && 
...
//   printf("Current packet ack number: %d\n", packet.acknum);
  ...
  {
      curseqnuma = packet.acknum;
      curacknuma = packet.seqnum + 1;
      stoptimer(0);
      altbit = 0;
    //   printf("Stop timer at a_input()\n");
...
int A_timerinterrupt() {
    // stoptimer(0); /* probably already stopped at timerinterrupt */
    // printf("Stop timer at interrupt()\n");
    tolayer3(0, buffera);
    starttimer(0, timerinc);
    return 0;
}
...
{
  // (void)packet;
  // check checksum and send to layer 5 curseqnumb == packet.seqnum && curacknumb== packet.acknum - 1 && 
//   printf("Current sequence number: %d\n", curseqnumb);
//   printf("Current ack number: %d\n", curacknumb);
//   printf("Current packet sequence number: %d\n", packet.seqnum);
//   printf("Current packet ack number: %d\n", packet.acknum);
